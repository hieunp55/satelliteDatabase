    ...
    if(all_overlays != undefined)
        for (var i = 0; i < all_overlays.length; i++){
            if(all_overlays[i].type == google.maps.drawing.OverlayType.RECTANGLE){
                ...
                var swBound = new google.maps.LatLng(neLatLg.lat(), neLatLg.lng());
                var neBound = new google.maps.LatLng(swLatLg.lat(), swLatLg.lng());
                bounds = new google.maps.LatLngBounds(neBound, swBound);
                return bounds;
            }
    }
    return bounds;
